...
layout(location = 0) out vec4 SSRResult;
...
void main() {
	// load basic data from texture and early quit
	vec4 RMLDValue = textureLod(RMLD, uv, 0.0f).rgba;
	float roughness = RMLDValue.r;
	float roughnessFade = GetRoughnessFade(roughness, roughnessThreshold);
	if(roughnessFade <= 0.0f){
		SSRResult = vec4(0.0f);
		return;
	}	
	vec3 N = texture(normal, uv).rgb;
	float linearDepth = RMLDValue.b;
	//	float deviceZ = texture(HiZ, uv, 0.0f).r;
	...
	if( roughness < roughnessSkipValue )
	{
		traceNumSteps = min( NumSteps * NumRays, 24u );
		traceNumRays = 1;
	}
...
//		StepOffset = 0.0f;
		...
		vec3 H = TangentBasis * importanceSampleGGX(E, a2);
		H = normalize(H);
...
//			vec4 hitColor = texture(SSRInput, finalHitUV);
			vec3 hitN = texture(normal, finalHitUV).rgb;
			vec4 hitColor = dot(hitN, L) < 0.0f ? textureLod(SSRInput, finalHitUV, 0.0f) : vec4(0.0f);
			colorSum += hitColor;
		}
	}
	
	vec4 outputColor = colorSum / max(float(traceNumRays), 0.00001f);
	SSRResult = vec4(outputColor.rgb * roughnessFade, 1.0f);
}